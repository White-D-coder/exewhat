import streamlit as st
import pandas as pd
import main
import io

st.set_page_config(page_title="WhatsApp Automation", page_icon="🟢")

st.title("🟢 WhatsApp Bulk Sender")
st.markdown("""
Upload your Excel file, scan the QR code, and send messages automatically.
**Note:** Use responsibly. Do not spam.
""")

# File Uploader
uploaded_file = st.file_uploader("Upload Excel File", type=["xlsx"])

if uploaded_file is not None:
    # Load Data
    try:
        df = pd.read_excel(uploaded_file)
        if 'Status' not in df.columns:
            df['Status'] = 'Pending'
        
        st.subheader("Data Preview")
        st.dataframe(df)
        
        # Stats
        total = len(df)
        sent = len(df[df['Status'] == 'Sent'])
        pending = total - sent
        
        st.metric("Pending Messages", pending, delta=f"Total: {total}")
        
        # Column Mapping
        st.subheader("Map Columns")
        col_map1, col_map2 = st.columns(2)
        
        # Try to auto-detect columns
        all_columns = df.columns.tolist()
        default_name_index = 0
        default_phone_index = 0
        
        for i, col in enumerate(all_columns):
            if "name" in col.lower(): default_name_index = i
            if "mobile" in col.lower() or "phone" in col.lower() or "contact" in col.lower(): default_phone_index = i

        with col_map1:
            name_col = st.selectbox("Select Name Column", all_columns, index=default_name_index)
        with col_map2:
            phone_col = st.selectbox("Select Phone Column", all_columns, index=default_phone_index)

        st.subheader("Message Configuration")
        col1, col2 = st.columns(2)
        with col1:
            custom_msg = st.text_area("Default Message (if not in Excel)", value="Hello {Name}, Join our community!")
            st.caption("Use {Name} as placeholder.")
        with col2:
            custom_link = st.text_input("Community Link", value="")
            st.caption("This will be appended to every message.")

        # Safety Settings
        with st.expander("🛡️ Safety Settings (Anti-Ban)", expanded=True):
            st.warning("Recommended: Keep delays higher to avoid ban.")
            s_col1, s_col2 = st.columns(2)
            with s_col1:
                min_d = st.number_input("Min Delay (sec)", min_value=5, value=10)
                batch_size = st.number_input("Batch Size (Pause after X msgs)", min_value=0, value=50, help="0 to disable")
            with s_col2:
                max_d = st.number_input("Max Delay (sec)", min_value=10, value=20)
                batch_pause = st.number_input("Batch Pause Duration (sec)", min_value=0, value=300, help="Time to wait after batch finishes")

        if st.button("🚀 Start Automation", disabled=(pending == 0)):
            status_text = st.empty()
            progress_bar = st.progress(0)
            log_area = st.empty()
            logs = []
            
            processed_count = 0
            
            # Run Automation
            for event_type, msg, data in main.process_messages(
                df, name_col, phone_col, 
                custom_message=custom_msg, custom_link=custom_link,
                min_delay=min_d, max_delay=max_d,
                batch_size=batch_size, batch_pause=batch_pause
            ):
                if event_type == "init":
                    status_text.info(msg)
                elif event_type == "login":
                    status_text.warning(msg)
                elif event_type == "progress":
                    status_text.text(msg)
                elif event_type == "update":
                    logs.append(msg)
                    log_area.text("\n".join(logs[-10:])) # Show last 10 logs
                    
                    # Update DataFrame
                    idx, status_val = data
                    df.at[idx, 'Status'] = status_val
                    processed_count += 1
                    
                    # Update progress
                    progress = min(processed_count / pending, 1.0) if pending > 0 else 1.0
                    progress_bar.progress(progress)
                    
                elif event_type == "error":
                    st.error(msg)
                elif event_type == "done":
                    st.success(msg)
                    status_text.empty()
            
            # Show updated dataframe
            st.session_state['df_result'] = df
            
    except Exception as e:
        st.error(f"Error reading file: {e}")

# Download Section
if 'df_result' in st.session_state:
    st.subheader("🎉 Result")
    st.dataframe(st.session_state['df_result'])
    
    # Convert DF to Excel for download
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        st.session_state['df_result'].to_excel(writer, index=False)
    
    st.download_button(
        label="📥 Download Updated Excel",
        data=output.getvalue(),
        file_name="updated_status.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )
